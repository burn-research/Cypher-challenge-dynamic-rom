# %% [markdown]
# # Tutorial 2: one PDE, three discretisations
#
...
def fd_rhs(u, x, D1, D2, nu, alpha):
    ...
    Burgers written out term by term:
        -u * (D1 @ u)      convection, nonlinear: u times its own slope
...
def assemble(x_q, w_q, Phi, dPhi):
    ...
    M = Phi.T @ (W[:, None] * Phi)            # <phi_m, phi_n>
    K = dPhi.T @ (W[:, None] * dPhi)          # <phi_m', phi_n'>
    B = Phi.T @ (W[:, None] * (h[:, None] * dPhi))   # <phi_m, h phi_n'>
    Lin = M - B                               # the  +v - h v_x  terms
    G = Phi.T @ (W * h)                       # <phi_m, h>
    F = Phi.T @ (W * source(x_q))             # <phi_m, f>

    # C[m, n, k] = - sum_q w_q phi_m phi_n dphi_k
    C = -np.einsum("q,qm,qn,qk->mnk", W, Phi, Phi, dPhi, optimize=True)
...
def reduced_rhs(op, a, nu, alpha):
    ...
    const = op["G"] + alpha * op["F"]
    linear = (op["Lin"] - nu * op["K"]) @ a
    quadratic = np.einsum("mnk,n,k->m", op["C"], a, a, optimize=True)
...
def spectral_basis(n_b, n_q=2001):
    ...
    basis and its derivative evaluated there.  The quadrature is separate from
    any solution grid: these integrals are exact objects, and n_q = 2001 makes
    the trapezoidal error negligible against everything else.
    """
    ...
    for n in range(1, n_b + 1):           # n is the wavenumber, from 1
        Phi[:, n - 1] = np.sqrt(2.0) * np.sin(n * np.pi * x_q)
        dPhi[:, n - 1] = np.sqrt(2.0) * n * np.pi * np.cos(n * np.pi * x_q)
...
# ## Answers
#
# **1. How many sine modes?** Sixteen.  The measured errors are $6.7\cdot10^{-2}$
# at $n_b=2$, $1.1\cdot10^{-3}$ at $8$, $7.4\cdot10^{-5}$ at $16$ and
# $3.3\cdot10^{-5}$ at $32$.  The last step buys almost nothing, because by then
# the error is no longer the spectral method's: it is the accuracy of the
# second-order reference itself.
#
# **2. Which curve is straight on which axis?**  The finite-element curve is
# straight on log-log, because algebraic convergence $e\sim\Delta x^p$ is a
# straight line there.  The spectral curve is straight on a *semilog* axis,
# because exponential convergence $e\sim e^{-cn_b}$ is.  Read the consequence
# rather than the shape: to gain a digit with finite elements you multiply the
# mesh, to gain a digit with a spectral basis you add a fixed number of modes.
#
# **3. Which lines changed between B and C?**  Only the construction of `Phi`
# and `dPhi` at the quadrature points.  `assemble` is called with different
# arguments and is not modified, and neither are the integrators.  Three
# methods, one derivation.
#
# **4. RK4 in place of the semi-implicit solver.**  $\Delta t$ falls to the
# diffusive stability limit of the finite-element operator, which scales like
# $\Delta x^2/\nu$.  With 50 elements and $\nu=0.03$ that is a few times
# $10^{-3}$, against the $2\cdot10^{-3}$ used here for accuracy, so the penalty
# is mild.  Refine the mesh once and it is not: the stability limit falls by
# four while the accuracy requirement falls by two.
