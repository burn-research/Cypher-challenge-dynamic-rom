# %% [markdown]
# # Tutorial 3: POD and a POD-Galerkin reduced-order model  (to fill)
#
...
#
# **There are eight TODO blocks.** The finite-difference model is given:
# it is Tutorial 2 part A, unchanged. Everything that is new to this
# tutorial, you write.
...
def weighted_pod(D, n_keep=None):
    ...
    """
    # TODO 1: the weighted SVD, in two lines.
    #   Take the SVD of W^{1/2} D, whose left singular vectors are orthonormal
    #   in the Euclidean sense, then undo the weighting so that the modes are
    #   orthonormal in the W_s inner product instead.  Check afterwards that
    #   Phi^T W_s Phi is the identity; if it is not, nothing below is right.
    #   sqrt_w[:, None] * D multiplies row i by sqrt(w_i), and
    #   full_matrices=False gives the thin SVD, which is all that exists.
    U, sigma, Vt = ...
    Phi = ...
...
def w_inner(A, B):
    ...
    Entry (m, n) is <a_m, b_n>_W.  Used for three things: checking
    orthonormality (should give the identity), computing the coefficients of a
    projection, and forming the reduced operators.
    """
    # TODO 2: one line.  Entry (m, n) must be sum_i w_i A[i, m] B[i, n].
    return ...
...
def global_basis(q=Q_PER_CLASS, tol=1e-10):
    ...
    # TODO 3: orthonormalise the stack, exactly as in TODO 1.
    ...
    #   Keep the columns whose singular value is above tol * s[0], and remember
    #   to unweight, as in TODO 1, so that Q^T W_s Q is the identity.
    U, s_overlap, _ = ...
    ...
    Q = ...                                          # unweight: spans the merge

    # ---- Step 2: rank the merged space by energy ---------------------------
    # TODO 4: Q is orthonormal but ordered by OVERLAP, which says nothing about
    #   how much data each direction explains.  Write every pooled snapshot in
    #   the Q basis (that is w_inner(Q, D_all), a p x n_t matrix), take the SVD
    #   of those coefficients, and rotate Q by the left singular vectors.
    #   A rotation of an orthonormal basis is still orthonormal, so only the
    #   labels change: same space, new order.
    Z = ...                               # (p, n_t): snapshot coords in Q
    Uz, s_energy, _ = ...
    return ..., s_overlap[keep], s_energy
...
def reduce_operators(Phi):
    ...
    # TODO 5: the four parameter-independent linear pieces.  Every one is a
    #   weighted sum over the grid, and WPhi already carries the weights, so
    #   each is a single matrix product.  WPhi.T @ vector gives
    #   sum_i w_i phi_m(x_i) vector(x_i) for every m, that is <phi_m, vector>.
    g_r = ...                             # <phi_m, h>: from the lifting
    f_r = ...                             # <phi_m, f>: from the source shape
    A_r = ...                             # <phi_m, phi_n - h phi_n'>
    B_r = ...                             # <phi_m, phi_n''>: diffusion
    #  ==> constant term = g_r + alpha f_r,  linear operator = A_r + nu B_r

    # TODO 6: the quadratic tensor,
    #   C_r[m, n, k] = - sum_i w_i phi_m(x_i) phi_n(x_i) phi_k'(x_i).
    #   np.einsum with the subscripts "i,im,in,ik->mnk" does it in one call:
    #   i is the grid index and is summed away, m, n and k survive.
    C_r = ...
...
def rom_terms(op, nu, alpha):
    ...
    """
    # TODO 7: combine the stored pieces into the constant vector, the linear
    #   matrix and the quadratic function.  For the last one, the subscripts
    #   "mnk,n,k->m" contract the tensor against a twice, giving
    #   component m = sum_{n,k} C[m,n,k] a_n a_k.
    const = ...                           # c_r(alpha), an (n_r,) vector
    L = ...                               # L_r(nu), an (n_r, n_r) matrix
    quad = lambda a: ...
    return const, L, quad
...
def projection_error(n_r):
    ...
    # TODO 8: project every reference snapshot onto the first n_r modes.
    #   The basis is W_s-orthonormal, so the projection is Phi (Phi^T W_s V),
    #   and w_inner is exactly that inner product.  Remember to add the
    #   lifting back before comparing with u.
    V_proj = ...
    u_proj = ...
